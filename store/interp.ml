...
type operand = Octets of string | Signed of string | KeyName of string
...
let bytes text = List.fold_right (fun c tail -> data "Bytes" 1 [Literal
  (Kanon_kernel.Literal.LInt (Kanon_kernel.Bignum.of_int (Char.code c))); tail])
...
let rec reply = function
  ...
  | Data (E.Tid "mu<Reply>", 1, [Data (E.Tid "mu<Signed64>", 0, [b])]) -> let* s = text b in
      Store.integer s |> Result.map_error Store.message |> Result.map (fun _ -> Int s)
...
let run ~budget rows ~entry store =
  ...
  let rec eval env term = let* () = poll () in
    ...
    | E.KCase (_tid, x, bs) -> let* x = eval env x in (match x with
        | Data (_t, tag, xs) -> let* b = List.find_opt (fun (b : E.kbranch) -> b.tag = tag) bs
            |> Option.to_result ~none:"STORE-CASE" in
  ...
  let rec script store value = let* () = poll () in match value with
    ...
    | Data (E.Tid "mu<Script>", tag, Data (E.Tid "mu<Key>", 0, [key]) :: args) -> let* key = text key in
        ...
        let bulk = finish (Option.fold ~none:(data "Reply" 0 []) ~some:(scalar 2)) in let status = finish (scalar 3) in
  ...
  let rec client store value = let* () = poll () in match value with
    ...
    | Data (E.Tid "mu<Client>", 1, [body; k]) ->
        ...
        let* () = match decoded with Err error -> Error error | Nil | Int _ | Bulk _ | Status _ | Array _ -> Ok () in
