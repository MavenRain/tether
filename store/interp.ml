...
type operand = Octets of string | Signed of string | KeyName of string | Condition of Store.expiry_condition | End of Store.side | BulkFields of string * string list | HashPairs of (string * string) * (string * string) list
...
let bytes text = List.fold_right (fun c tail -> data "Bytes" 1 [Literal (Kanon_kernel.Literal.LInt (Kanon_kernel.Bignum.of_int (Char.code c))); tail]) (List.of_seq (String.to_seq text)) (data "Bytes" 0 [])
...
let operand = function
  ...
  | Data (E.Tid "mu<ListEnd>", tag, []) -> List.assoc_opt tag [0, Store.Left; 1, Store.Right] |> Option.to_result ~none:"STORE-LIST-END" |> Result.map (fun side -> End side)
  ...
  let rec script store value = let* () = poll () in match value with
    ...
    | Data (E.Tid "mu<Script>", tag, Data (E.Tid "mu<Key>", 0, [key]) :: args) -> let* key = text key in
        ...
        let* answer, store = match tag, args with
          ...
          | 5, [Signed s] -> integer (Store.incrby key s store) | 71, [Octets s] -> integer (Store.append key s store)
          | (7 | 8), [] -> integer (Ok (if tag = 7 then Store.del key store else (Store.exists key store, store)))
          | (9 | 59), [Octets f; Octets v] -> integer (Store.hset ~nx:(tag = 59) key f v store) | 58, [HashPairs ((f, v), rest)] -> integer (Store.hset ~rest key f v store)
          | 10, [Octets f] -> bulk (keep (Store.hget key f store)) | (11 | 12 | 60), [Octets f] -> integer (if tag = 11 then Store.hdel key f store else keep ((if tag = 12 then Store.hexists else Store.hstrlen) key f store))
          | (13 | 18 | 23 | 72), [] -> integer (keep ((match tag with 13 -> Store.hlen | 18 -> Store.scard | 23 -> Store.llen | _ -> Store.strlen) key store))
