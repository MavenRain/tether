...
MUTANTS = [
    ...
    ('STORE-DESTINATION', 'store/store.ml', b'save destination (Set values) ~empty:(values = []) (remove destination store)', b'save key (Set values) ~empty:(values = []) (remove key store)', UNIT, b'FAIL SET-STORE-UNIT missing'),
    ...
    ('STORE-ALIAS', 'store/store.ml', b'let* values = scombine op key other store in', b'let* values = scombine op key other (remove destination store) in', UNIT, b'FAIL SET-STORE-UNIT left only'),
