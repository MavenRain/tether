#!/usr/bin/env python3
"""Variadic Hash field/value writes across the independent store, twin and live Redis."""
...
spec = importlib.util.spec_from_file_location('hset_many_support', ROOT / 'dev/lists-tests.py')
support = importlib.util.module_from_spec(spec)
spec.loader.exec_module(support)
run, require, tet = support.run, support.require, support.tet
TAG, KEY, WRONG = support.TAG, support.KEY, support.WRONG
VALUES = [(b'a', b'A'), (b'b', b'B'), (b'a', b'last'), (b'c', b'C'), (b'', b'')]
QUERY = {
    'write': ('hsetMany', VALUES), 'one': ('hsetMany', [(b'one', b'value')]),
    'many': ('hsetMany', [(f'v{i:03}'.encode(), str(i).encode()) for i in range(129)]),
    'binary': ('hsetMany', [(bytes(range(256)), support.BINARY), (b'\x00', bytes(range(256))), (b'', b'')]),
    'computed': ('hsetMany', VALUES), 'within': ('hsetMany', VALUES),
    'snapshot': ('hsetMany', VALUES), 'raw': ('hsetMany', VALUES),
}
ENTRIES = tuple(QUERY) + ('earlier',)
EXAMPLES = {'main': b'["name","Alice","role","member","visits","9007199254740993"]\n',
            'added': b'3\n', 'retained': b'3\n'}


def arguments(values):
    require(bool(values), 'HSET-MANY nonempty test arguments')
    field, value = values[-1]
    result = f'(pairOne {tet(field)} {tet(value)})'
    for field, value in reversed(values[:-1]):
        result = f'(pairMore {tet(field)} {tet(value)} {result})'
    return result
...
def cases():
    inputs = [
        ('write', None), ('write', {b'a': b'old'}), ('write', {b'old': b'kept'}),
        ('write', dict(VALUES)), ('write', {b'a': b'old', b'b': b'prior'}), ('write', {b'': b'old'}),
        ('one', None), ('one', {b'one': b'old'}), ('one', {b'old': b'kept'}),
        ('many', None), ('many', {b'v000': b'old', b'v128': b'old', b'old': b'kept'}),
        ('many', dict(QUERY['many'][1])), ('binary', None), ('binary', {bytes(range(256)): b'old', b'': b'old'}),
        ('computed', {b'a': b'old', b'kept': b'K'}),
        ('within', {b'a': b'old', b'b': b'prior', b'old': b'K'}),
        ('earlier', {b'a': b'old', b'b': b'prior', b'old': b'K'}),
        ('snapshot', {support.TEXT: b'kept', b'quote': support.TEXT}),
        ('snapshot', {b'kept': b'\xff'}), ('raw', {b'a': b'old'}),
    ]
    inputs += [('write', value) for value in (b'wrong', {b'm'}, [b'm'])]
    rows = []
    for entry, initial in inputs:
        if initial is not None and not isinstance(initial, dict):
            ...
            continue
        _, values = QUERY['write' if entry == 'earlier' else entry]
        old = initial or {}
        after = dict(old)
        after.update(values)
        changed = len(set(field for field, _ in values) - old.keys())
    ...
    require(len(rows) == 23, 'HSET-MANY case inventory')
    return rows


def source(seed=None):
    declarations = ['module HsetManyCases', 'schema jobs : String -> Key Hash tag b"queue"',
        'def identity : Bytes -> Bytes := fun (b : Bytes) => b',
        'def keepPairs : BulkPairs -> BulkPairs := fun (args : BulkPairs) => args',
    ...
    for entry, (name, values) in QUERY.items():
        ...
        if entry == 'computed':
            cmd = cmd.replace(arguments(values),
                f'(pairMore (identity {tet(values[0][0])}) (identity {tet(values[0][1])}) (keepPairs {arguments(values[1:])}))')
    ...
    declarations += [f'def earlier : Client Reply := do {{ r <- inv Reply {TAG} writeScript; '
...
def twin(work, output, initial, expected, after, kind):
    try:
        ...
        raise AssertionError('HSET-MANY LuaJIT reply' if str(error) == 'LISTS LuaJIT reply' else str(error)) from None
...
def offline(work, outputs):
    for i, (entry, initial, expected, after, kind) in enumerate(cases()):
        ...
        (work / 'HsetManyCases.tet').write_text(source(i))
        rows = []
        code = support.checker.check(work, 'HsetManyCases.tet', command=[str(ROOT / '_build/default/dev/store_run.exe'),
            'HsetManyCases.tet', 'seeded' if isinstance(initial, dict) else entry, KEY, seed, '1000000'], receive=rows.append)
        require(code == 0 and rows == [b'REPLY ' + wire(expected, kind) + b'\n'], f'HSET-MANY store reply {i}: {rows!r}')
    print('PASS HSET-MANY-ORACLES store=23 luajit=23', flush=True)


def refusals(work):
    mismatch = b'CHECK mismatch: the constructor keyBytes of Key gives the index '
    cmd = command('hsetMany', [(b'a', b'A')])
    ...
    invalid += [(cmd.replace(arguments([(b'a', b'A')]), bad), 'Hash', reason) for bad, reason in [
        ('b"a"', b'CHECK unbound: bytesCons is not a constructor of BulkPairs'),
        ('repliesNil', b'CHECK unbound: repliesNil is not a constructor of BulkPairs'),
        ('(bulkOne b"a")', b'CHECK unbound: bulkOne is not a constructor of BulkPairs'),
        ('(pairOne nil b"A")', b'CHECK unbound: nil is not a constructor of Bytes'),
        ('(pairOne b"a" nil)', b'CHECK unbound: nil is not a constructor of Bytes'),
        ('(pairMore b"a" b"A" (bulkOne b"b"))', b'CHECK unbound: bulkOne is not a constructor of BulkPairs'),
        ('(pairOne b"a")', b'CHECK mismatch: pairOne takes 2 arguments and the term gives 1'),
        ('(pairMore b"a" b"A" b"b")', b'CHECK unbound: bytesCons is not a constructor of BulkPairs'),
    ]]
    for i, (body, schema, diagnostic) in enumerate(invalid):
        ...
        require(result.returncode == 2 and diagnostic in result.stderr, f'HSET-MANY refusal {i}: {result.stderr!r}')
        require(not output.exists(), f'HSET-MANY refusal published output {i}')
    require(len(invalid) == 14, 'HSET-MANY refusal inventory')
    print('PASS HSET-MANY-REFUSALS cases=14 atomic_output=14', flush=True)
...
def live(work, outputs):
    ...
    with support.local.hosts(work) as (port, env):
        ...
        live_cases = cases() + [(entry, kind, WRONG, kind, 'status')
                                for entry in ('write',) for kind in ('zset', 'stream')]
        for entry, initial, expected, after, kind in live_cases:
            for host in ('node', 'bash'):
                ...
                try:
                    ...
                    wanted, code = b'', 4
                require(run(args, env=env, code=code) == wanted, f'HSET-MANY {host} {entry} reply')
                if isinstance(after, dict):
                    ...
                    require(run(redis + ['EVAL', check, '1', KEY]) == b'1\n', 'HSET-MANY live complete hash')
                    require(run(redis + ['PEXPIRETIME', KEY]) == (b'-1\n' if initial is None else deadline), 'HSET-MANY preserves expiry')
                elif after is None:
                    require(run(redis + ['EXISTS', KEY]) == b'0\n',
                            'HSET-MANY retained reply survives deletion' if entry in ('within', 'earlier')
                            else 'HSET-MANY deleted key stays absent')
                else:
                    require(run(redis + ['DUMP', KEY]) == before and run(redis + ['PEXPIRETIME', KEY]) == deadline,
                            'HSET-MANY wrong type preserves complete state')
                require(run(redis + ['GET', 'other']) == b'kept\n', 'HSET-MANY unrelated key')
        ...
        for entry in ('raw',):
            for host in ('node', 'bash'):
                ...
                args = (['node', 'runtime/redis-host.mjs', str(output / 'prog.wasm'), str(port)]
                        if host == 'node' else ['/bin/bash', str(output / 'prog.sh')])
                require(run(args, env=env, code=4) == b'', 'HSET-MANY error stops host')
                require(run(redis + ['GET', KEY]) == b'wrong\n', 'HSET-MANY error preserves value')
                hosts, errors = hosts + 1, errors + 1
    require((hosts, rejected, errors) == (52, 2, 2), 'HSET-MANY host inventory')
    print(f'PASS HSET-MANY-E2E cases={len(live_cases)} hosts={hosts} utf8_refusals={rejected} errors={errors}', flush=True)
    count = 0
    for entry, expected in EXAMPLES.items():
        for host in ('node', 'bash', 'luajit'):
            require(run(['./tether', 'exec', 'examples/ProfileBatch.tet', '--entry', entry, '--host', host]) == expected,
                    f'HSET-MANY example {entry} {host}')
            count += 1
    print(f'PASS HSET-MANY-EXAMPLE exec={count}', flush=True)


def main():
    probe = len(sys.argv) == 3 and sys.argv[1] == '--probe'
    require(probe or sys.argv[1:] in ([], ['--static'], ['--offline']), 'Usage: hset-many-tests.py [--static|--offline|--probe ENTRY]')
    with tempfile.TemporaryDirectory(prefix='tether-hset-many-') as temporary:
        work = Path(temporary)
        path = work / 'HsetManyCases.tet'
        path.write_text(source())
        if probe:
            entry = sys.argv[2]
            rows = [row for row in cases() if row[0] == entry]
            require(entry in ENTRIES and bool(rows), 'HSET-MANY probe must have cases')
            output = emit(work, path, entry)
            for _entry, initial, expected, after, kind in rows:
                twin(work, output, initial, expected, after, kind)
            print(f'PASS HSET-MANY-PROBE entry={entry} cases={len(rows)}', flush=True)
            return
        outputs = {entry: emit(work, path, entry) for entry in ENTRIES}
        print(f'PASS HSET-MANY-ARTIFACTS pairs={len(outputs)}', flush=True)
        refusals(work)
        for entry, expected in EXAMPLES.items():
            require(run(['./tether', 'run', 'examples/ProfileBatch.tet', '--entry', entry]) == expected, 'HSET-MANY interpreter example')
        if '--static' not in sys.argv:
            offline(work, outputs)
        if not sys.argv[1:]:
            live(work, outputs)
    print('PASS HSET-MANY-TESTS' + (f' mode={sys.argv[1][2:]}' if sys.argv[1:] else ''), flush=True)
...
if __name__ == '__main__':
    try:
        ...
        print(f'FAIL HSET-MANY-TESTS {error}', file=sys.stderr)
        sys.exit(1)
