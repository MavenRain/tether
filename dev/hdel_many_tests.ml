...
let require condition reason = if condition then Ok () else Error ("HDEL-MANY-UNIT " ^ reason)
...
let execute ?(expose=false) args before =
  ...
  let script = E.KTag (E.Tid "mu<Script>", 57, key :: args @ [E.KClos (E.Fid "pure", 1, [])]) in
...
let fields names = List.map (fun name -> name, "value:" ^ name) names
let run () =
  ...
  let cases = [
    ...
    Some ("kept" :: many), "v000", many, ["kept"], "129";
    Some ["a"; "b"], "missing", ["b"], ["a"], "1";
    Some ["a"; "b"], "a", [], ["b"], "1"] in
  let* values = fold (fun (initial, first, rest, expected, count, expiring) ->
    let before = S.put "other" (S.Str "kept") S.empty in
    let* _, before = store (S.expire ~seconds:false "other" "9000" before) in
    let before = Option.fold ~none:before ~some:(fun xs -> S.put "k" (S.Hash (fields xs)) before) initial in
    let* before = if expiring then Result.map snd (store (S.expire ~seconds:false "k" "5000" before)) else Ok before in
    let wanted = match expected with [] -> S.remove "k" before | xs -> S.put "k" (S.Hash (fields xs)) before in
    let* actual = store (S.hdel ~rest "k" first before) in
    let* () = require (actual = (count, wanted)) "store count and complete state" in
    let* reply, after = execute [arguments first rest] before in
  ...
  let* errors = fold (fun value ->
    ...
    let* () = require (S.hdel ~rest:["b"; "c"] "k" "a" before = Error S.Wrong_type) "store wrong type" in
    let* () = require (execute [arguments "a" ["b"; "c"]] before = Error (S.message S.Wrong_type)) "error stops client" in
    let* reply, after = execute ~expose:true [arguments "a" ["b"; "c"]] before in
  ...
  let* reply, after = execute [arguments "a" []] expired in
  let* () = require (reply = I.Int "0" && after = expired) "expired key is absent and has no expiry" in
  let count = values + errors + shapes + 1 in
  let* () = require (count = 36) "case inventory" in Ok count
let () = run () |> Result.fold
  ~ok:(fun count -> Printf.printf "PASS HDEL-MANY-UNIT cases=%d\n" count)
