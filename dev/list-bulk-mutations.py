...
MUTANTS = [
    ...
    ('STORE-TYPE', 'store/store.ml', b'let push ?(rest = []) side key value store = let* values = list key store in',
     b'let push ?(rest = []) side key value store = let values = Result.value ~default:[] (list key store) in',
     UNIT, b'FAIL LIST-BULK-UNIT store wrong type'),
    ('INTERPRETER-DIRECTION', 'store/interp.ml', b'if tag = 53 then Store.Left else Store.Right',
     b'if tag = 53 then Store.Right else Store.Left', UNIT, b'FAIL LIST-BULK-UNIT interpreter order, count and complete state'),
    ('INTERPRETER-ARGS', 'store/interp.ml', b'Store.push ~rest (if tag = 53',
     b'Store.push ~rest:(List.filter (fun _ -> false) rest) (if tag = 53', UNIT, b'FAIL LIST-BULK-UNIT interpreter order, count and complete state'),
    ('INTERPRETER-STATE', 'store/interp.ml', b'integer (Store.push ~rest (if tag = 53 then Store.Left else Store.Right) key first store)',
     b'integer (Result.map (fun (count, _) -> count, Store.empty) (Store.push ~rest (if tag = 53 then Store.Left else Store.Right) key first store))',
     UNIT, b'FAIL LIST-BULK-UNIT interpreter order, count and complete state'),
    ('LUA-DIRECTION', 'print/lua.ml', b"s.tag == 53 and 'LPUSH' or 'RPUSH'",
     b"s.tag == 53 and 'RPUSH' or 'LPUSH'", probe('right'), b'TWIN list order mismatch'),
    ...
    ('TWIN-DIRECTION', 'dev/lua-store.lua', b"command == 'LPUSH' and 1 or #items + 1, item",
     b"command == 'RPUSH' and 1 or #items + 1, item", probe('right'), b'TWIN list order mismatch'),
