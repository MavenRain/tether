...
MUTANTS = [
    ...
    ('SREM-EMPTY-KEY', 'store/store.ml', b'if Members.is_empty values then Keys.remove key store',
     b'if Members.is_empty values then (if Keys.mem key store then put key (Set []) store else store)',
