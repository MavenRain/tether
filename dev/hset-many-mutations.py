...
MUTANTS = [
    ...
    ('STORE-TYPE', 'store/store.ml', b'let hset ?(nx = false) ?(rest = []) key field value store = let* fields = hash key store in',
     b'let hset ?(nx = false) ?(rest = []) key field value store = let fields = Result.value ~default:Keys.empty (hash key store) in', UNIT, b'FAIL HSET-MANY-UNIT store wrong type'),
