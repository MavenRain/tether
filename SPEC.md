...
M1 String byte slice, 2026-09-21. The driver emits the executable Wasm
...
APPEND adds bytes to Binary Strings without clearing existing expiry;
STRLEN reads the byte length of either String encoding. See
`dev/STRING-BYTES.md` for typing, empty values and the default size limit.
...
LREM removes matching List values with exact signed 64-bit counts.
See `dev/LIST-REMOVE.md` for direction, cleanup and retained replies.
Typed LINSERT BEFORE and AFTER use the first matching pivot, preserving
List order and expiry. See `dev/LIST-INSERT.md` for replies and validation.
LMOVE transfers one element between List keys or rotates a List with
typed left/right endpoints. See `dev/LIST-MOVE.md` for atomicity and expiry.
LPOP and RPOP with a signed 64-bit count remove up to that many values in
pop order and return a nullable array. See `dev/LIST-POP.md` for zero,
...
`saddMany` and `sremMany` change a nonempty `BulkArgs` member list in one
command. See `dev/SET-BULK.md` for distinct counts, deletion and expiry.
`hdelMany` deletes a nonempty `BulkArgs` field list in one Hash command.
See `dev/HDEL-MANY.md` for distinct counts, deletion and retained replies.
`hsetMany` writes a nonempty `BulkPairs` list in one Hash command.
See `dev/HSET-MANY.md` for paired fields and values, overwrite order and counts.

`hsetnx` sets a Hash field only when absent, including when its existing
value is empty. `hstrlen` reads its byte length and returns zero for a
missing key or field. See `dev/HASH-CONDITIONAL.md` for write classification,
expiry preservation and the profile-default example.
...
`dev/PRELUDES.sha256`; their 197 lines are reported separately without
...
access, trimming, range replies, bulk and conditional pushes, value removal,
pivot insertion, atomic List moves, bulk List pops, String appends and byte lengths, bulk Set addition and removal,
...
profile-default, active-queue, queue-cleanup, queue-insertion, queue-transfer, queue-drain and string-buffer examples are implemented.
