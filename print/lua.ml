...
local function run(s) while s.tag ~= 0 do
    ...
    elseif (s.tag >= 7 and s.tag <= 13) or (s.tag >= 15 and s.tag <= 20) or s.tag == 23 or (s.tag >= 35 and s.tag <= 51) or (s.tag >= 53 and s.tag <= 67) or s.tag == 71 or s.tag == 72 then
      ...
      elseif s.tag == 65 then got = redis.pcall('LREM',k,text(s[2][1]),text(s[3])); next = s[4] elseif s.tag == 71 then got = redis.pcall('APPEND',k,text(s[2])); next = s[3]
      ...
      elseif s.tag == 13 or s.tag == 18 or s.tag == 23 or s.tag == 72 then got = redis.pcall(({[13]='HLEN',[18]='SCARD',[23]='LLEN',[72]='STRLEN'})[s.tag],k)
      ...
      elseif type(got) ~= 'number' then local what = s.tag >= 71 and 'string length' or s.tag == 65 and 'removed count' or (s.tag == 7 or s.tag == 8) and 'key count' or ((s.tag == 17 or s.tag == 38) and 'membership'
